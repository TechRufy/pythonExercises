...
class Strada:
    ...
    def __init__(self, auto, tratte):
        self.listAuto = {}
        self.listBus = {}
        for nomi in auto:
            car = Auto(nomi)
            self.listAuto[nomi] = car

        for nomi, vie in tratte:
            bus = Autobus(nomi, vie)
            self.listBus[nomi] = bus
        manovratore = Manovratore(self.listAuto, self.listBus)

    def getVeicolo(self, nome):
        try:
            return self.listAuto[nome]

        except KeyError:
            return self.listBus[nome]


class Manovratore:

    def __init__(self, listAuto, listBus):
        for auto in listAuto.values():
            auto.manovratore = self
        for bus in listBus.values():
            bus.manovratore = self

    def aziona(self, veicolo):
        funzioni = {"sonoARiserva": self.azioneAuto,
                    "arrivaAllaFermata": self.azioneAutobus,
                    "arrivaAlCapolinea": self.azioneAutobusC}
        func = inspect.stack()[1].function
        funzioni[func](veicolo)

    def azioneAuto(self, veicolo):
        print("L' auto {} sta facendo benzina ".format(veicolo.id_veicolo))
        veicolo.reset_spia()

    def azioneAutobus(self, veicolo):
        print("L'autobus {0} sta effetuando una fermata {1} ".format(veicolo.id_veicolo,
                                                                     veicolo.aggiorna_ora_ultima_fermata()))

    def azioneAutobusC(self, veicolo):
        print("L’autobus {} è arrivato al capolinea: ora il suo tragitto è {}.".format(veicolo.id_veicolo,
                                                                                       veicolo.aggiorna_direzione()))
